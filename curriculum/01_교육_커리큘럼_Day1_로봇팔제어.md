...
```mermaid
graph LR
    HW[하드웨어<br/>조립] --> TEST[단계별<br/>테스트]
    TEST --> AUTO[자동화<br/>EEPROM]
    AUTO --> BT[원격 제어<br/>Bluetooth]
    BT --> VOICE[음성 제어<br/>AI <br/> STT/TTS]
...
```mermaid
graph LR
    BASE[베이스<br/>플레이트] --> SERVO1[1축 서보<br/>고정]
    SERVO1 --> ARM1[첫번째<br/>팔 연결]
...
```mermaid
graph LR
    ARM1[1번 팔<br/>어깨] --> SERVO2[2축 서보<br/>장착]
    SERVO2 --> ARM2[2번 팔<br/>팔꿈치]
    ARM2 --> SERVO3[3축 서보<br/>장착]
    SERVO3 --> GRIPPER[그리퍼<br/>연결]
...
```mermaid
graph TD
    ARD[아두이노<br/>UNO] --> PWR[전원<br/>5V GND]
    ARD --> S1[D4: 베이스]
    ARD --> S2[D5: 어깨]
    ARD --> S3[D6: 팔꿈치]
    ARD --> S4[D7: 그리퍼]
    
    ARD --> JOY1[A0-A1:<br/>조이스틱1]
    ARD --> JOY2[A2-A3:<br/>조이스틱2]
    
    ARD --> BT[RX-TX:<br/>블루투스]
...
```mermaid
graph LR
    PWR[5V 2A<br/>어댑터] --> ARD[아두이노<br/>VIN]
    ARD --> SERVO[서보모터<br/>4개]
...
```mermaid
graph LR
    START[시작] --> INIT[서보 초기화<br/>attach 4개]
    INIT --> HOME[홈 포지션<br/>90,90,90,5]
    HOME --> TEST1[1축 테스트<br/>0→180]
    TEST1 --> WAIT1[1초 대기]
    WAIT1 --> TEST2[2축 테스트]
    TEST2 --> WAIT2[1초 대기]
    WAIT2 --> TEST3[3축 테스트]
...
```mermaid
graph LR
    START[시작] --> READ[조이스틱<br/>아날로그 읽기]
    READ --> MAP[0-1023을<br/>0-180으로 변환]
    MAP --> SMOOTH[이동평균<br/>필터 적용]
    SMOOTH --> WRITE[서보에<br/>각도 전송]
    WRITE --> DELAY[20ms 대기]
    DELAY --> READ
...
```mermaid
graph TD
    START[명령 수신] --> CHECK{시작이<br/>arm?}
    CHECK -->|Yes| INDEX[인덱스<br/>추출]
    CHECK -->|No| OTHER[다른 명령<br/>체크]
    
    INDEX --> ANGLE[각도값<br/>추출]
    ANGLE --> VALID{유효한<br/>범위?}
    VALID -->|Yes| MOVE[서보<br/>이동]
    VALID -->|No| ERROR[에러<br/>응답]
...
```mermaid
graph TD
    EEPROM[EEPROM<br/>1024 bytes] --> HEADER[주소 0: 개수<br/>savedCount]
    EEPROM --> STEP1[주소 4-7: Step 1<br/>4개 각도]
    EEPROM --> STEP2[주소 8-11: Step 2<br/>4개 각도]
    EEPROM --> STEPN[...<br/>최대 12단계]
    
    HEADER -.예시.-> H1[EEPROM.read0 = 3<br/>저장된 위치 3개]
    STEP1 -.예시.-> S1[주소 4: 90°<br/>주소 5: 80°<br/>주소 6: 90°<br/>주소 7: 30°]
...
```mermaid
graph TD
    START[save 명령 수신] --> READ_COUNT[EEPROM 주소 0<br/>읽기 → savedCount]
    READ_COUNT --> CHECK{savedCount<br/><12?}
    
    CHECK -->|No| FULL[ERR: MEMORY_FULL<br/>최대 12개 저장 가능]
    CHECK -->|Yes| CALC_ADDR[저장 주소 계산<br/>addr=4+savedCount×4]
    
    CALC_ADDR --> DETACH[servo.detach<br/>타이머 충돌 방지]
    DETACH --> WRITE_LOOP[EEPROM.write<br/>4개 각도 저장]
    
    WRITE_LOOP --> WRITE_COUNT[EEPROM.write0<br/>savedCount+1]
    WRITE_COUNT --> ATTACH[servo.attach<br/>재연결]
    
    ATTACH --> PRINT[OK: SAVE_#n<br/>저장 완료 출력]
...
```mermaid
graph TD
    START[play 명령 수신] --> READ_COUNT[EEPROM 주소 0<br/>읽기 → savedCount]
    READ_COUNT --> CHECK{savedCount<br/>>0?}
    
    CHECK -->|No| NODATA[ERR: NO_DATA<br/>저장된 위치 없음]
    CHECK -->|Yes| PRINT_START[PLAY_START<br/>출력]
    
    PRINT_START --> INIT_LOOP[step = 0]
    INIT_LOOP --> LOOP_CHECK{step<br/><savedCount?}
    
    LOOP_CHECK -->|Yes| CALC_ADDR[주소 계산<br/>addr=4+step×4]
    CALC_ADDR --> READ_ANGLES[EEPROM에서<br/>4개 각도 읽기]
    
    READ_ANGLES --> MOVE_SERVO[서보 모터<br/>4개 이동]
    MOVE_SERVO --> PRINT_STEP[STEP_n 출력]
    PRINT_STEP --> WAIT[1000ms 대기]
    
    WAIT --> INC_STEP[step++]
    INC_STEP --> LOOP_CHECK
    
    LOOP_CHECK -->|No| DONE[PLAY_DONE<br/>재생 완료]
...
```mermaid
graph TD
    START[auto 명령 수신] --> SET_FLAG[autoRepeat = true<br/>repeatCount = 0]
    SET_FLAG --> PRINT_START[AUTO_ON<br/>출력]
    
    PRINT_START --> LOOP_MAIN[메인 루프]
    LOOP_MAIN --> CHECK_FLAG{autoRepeat<br/>==true?}
    
    CHECK_FLAG -->|No| WAIT_CMD[명령 대기]
    WAIT_CMD --> LOOP_MAIN
    
    CHECK_FLAG -->|Yes| CHECK_SERIAL[Serial 명령<br/>체크]
    CHECK_SERIAL --> IS_STOP{stop<br/>명령?}
    
    IS_STOP -->|Yes| STOP_FLAG[autoRepeat = false]
    STOP_FLAG --> PRINT_STOP[AUTO_OFF<br/>반복 횟수 출력]
    PRINT_STOP --> LOOP_MAIN
    
    IS_STOP -->|No| INC_COUNT[repeatCount++]
    INC_COUNT --> PRINT_REPEAT[반복 #n 출력]
    PRINT_REPEAT --> PLAY_SEQ[playSequence<br/>호출]
    
    PLAY_SEQ --> DELAY_500[500ms 대기]
    DELAY_500 --> LOOP_MAIN
...
## 5교시: 📡 05~06단계 - Bluetooth 원격 제어 ⭐⭐ (1.5시간)

**학습 목표**: 스마트폰으로 원격 제어 + 안정성 개선

### Bluetooth 통신 구조

```mermaid
sequenceDiagram
    participant Phone as 스마트폰<br/>앱
    participant BT as HC-06<br/>블루투스
    participant Arduino as 아두이노
    participant Servo as 로봇팔
    
    Phone->>BT: save_ 명령
    BT->>Arduino: Serial 수신
    Arduino->>Arduino: 명령 버퍼링
    Arduino->>Servo: 현재 위치 저장
    Servo-->>Arduino: 완료
    Arduino->>BT: OK:SAVE
    BT->>Phone: 응답 표시
```
...
```mermaid
graph TD
    START[Bluetooth<br/>데이터 수신] --> AVAILABLE{Serial<br/>available?}
    AVAILABLE -->|No| CONTINUE[다른 작업]
    AVAILABLE -->|Yes| READ_CHAR[char c = read]
    
    READ_CHAR --> PRINT_CHAR[BT 수신: c<br/>ASCII 출력]
    PRINT_CHAR --> APPEND[btBuffer += c<br/>버퍼에 추가]
    
    APPEND --> CHECK_TERMINATOR{c == '_'<br/>?}
    CHECK_TERMINATOR -->|No| AVAILABLE
    
    CHECK_TERMINATOR -->|Yes| TRIM[공백 제거<br/>trim]
    TRIM --> PRINT_CMD[명령 버퍼:<br/>btBuffer 출력]
    
    PRINT_CMD --> PARSE{명령 인식}
    PARSE -->|save| CMD_SAVE[savePosition<br/>호출]
    PARSE -->|play| CMD_PLAY[playSequence<br/>호출]
    PARSE -->|auto| CMD_AUTO[autoRepeat=true]
    PARSE -->|stop| CMD_STOP[autoRepeat=false]
    PARSE -->|clear| CMD_CLEAR[EEPROM 초기화]
    PARSE -->|list| CMD_LIST[목록 출력]
    PARSE -->|Unknown| CMD_ERR[ERR: UNKNOWN]
    
    CMD_SAVE & CMD_PLAY & CMD_AUTO & CMD_STOP & CMD_CLEAR & CMD_LIST & CMD_ERR --> CLEAR_BUF[btBuffer = ""<br/>버퍼 클리어]
...
void loop() {
  ...
  while (Serial.available() > 0) {
    ...
    if (c == '_') { // 명령 종료 문자
      // 명령어 끝난 자리에서 공백 제거 (트림 동작)
      btBuffer[btPos] = '\0';
      // 좌우 공백 트림
      char* cmd = btBuffer;
      while (*cmd == ' ' || *cmd == '\t') cmd++;       // 앞 공백
      for (int i = strlen(cmd) - 1; i >= 0; i--) {     // 뒤 공백
        if (cmd[i] == ' ' || cmd[i] == '\t') cmd[i] = '\0';
        else break;
      }

      Serial.println("[명령 종료 문자: '_']");
      Serial.print("명령어 버퍼: \"");
      Serial.print(cmd);
      Serial.print("\" (길이: ");
      ...
      if (strcmp(cmd, "save") == 0) {
        ...
        autoRepeat = true;
        Serial.println("OK:AUTO_ON");
      } else if (strcmp(cmd, "stop") == 0) {
        autoRepeat = false;
        Serial.println("OK:AUTO_OFF");
...
sequenceDiagram
    ...
    App->>App: 명령 파싱<br/>"저장" 감지
    App->>TTS: "저장 명령을 전송합니다"
    TTS-->>User: 음성 재생
    
    App->>BT: "save_" 전송
    BT->>Arduino: Serial 수신
    
    Arduino->>Arduino: 버퍼링<br/>'s''a''v''e''_'
    Arduino->>Servo: 현재 각도 읽기
    Arduino->>Arduino: EEPROM 저장
    
    Arduino->>BT: "OK:SAVE_3" 응답
    BT->>App: 응답 수신
    
    App->>App: 응답 파싱<br/>"OK:SAVE" 감지
    App->>TTS: "저장이 완료되었습니다"
    TTS-->>User: 음성 재생
    
    App->>App: UI 업데이트<br/>상태 표시
```
...
stateDiagram-v2
    ...
    Listening --> Processing: STT 결과 수신
    Processing --> Sending: 명령 파싱 완료
    
    Sending --> WaitingResponse: BT 전송 완료
    WaitingResponse --> Connected: 응답 수신<br/>(OK/ERR)
    WaitingResponse --> Connected: 타임아웃 (3초)
    
    Connected --> ManualControl: 버튼 클릭
    ManualControl --> Sending: 명령 전송
    
    Connected --> AutoMode: auto_ 전송
    AutoMode --> Connected: stop_ 전송
...
```mermaid
graph LR
    USER[사용자<br/>음성] --> STT[Speech<br/>Recognizer]
    STT --> PARSE[명령<br/>파싱]
    PARSE --> BT[Bluetooth<br/>전송]
    BT --> ARM[로봇팔<br/>실행]
    ARM --> RESP[응답<br/>수신]
